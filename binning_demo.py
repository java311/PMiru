...
camera.capture_video_frame(filename='bin1.jpg', timeout=-1)

cam_info = camera.get_camera_property()
print (cam_info['SupportedBins'])
for b in (1, 2, 3):  # cam_info['SupportedBins']:
    print('Testing binning=%d' % b)
    camera.set_roi(bins=b)
    print (camera.get_roi())
    camera.capture_video_frame(filename='bins_%d.jpg' % b, timeout=-1)
